            ...
            <div class="overflow-x-auto">
                <table class="table-auto w-full">
                    ...
                    <tbody class="text-sm divide-y divide-slate-200">
                        <Documents v-for="document in documents" :key="document.id" :document="document"
                            v-model:selected="selected" :value="document.id" />
                    </tbody>
                </table>
...
import Documents from './DocumentsTableItem.vue';
...
const checkAll = () => {
    selected.value = []
    if (!selectAll.value) {
        selected.value = documents.value.map(document => document.id)
    }
}

watch(selected, () => {
    selectAll.value = documents.value.length === selected.value.length ? true : false
